...
public class HuffmanTreeCoder {
     ...
     * 
     * @param node
     * @param prefix to be added to before every char code constructed from the
     *               subtree
     */
    ...
    public void buildTree(Map<Character, Double> freqMap) {
        ...
        while (!queue.isEmpty()) {
            HuffmanTreeNode rem1 = queue.remove();

            if (queue.isEmpty()) {
                root = rem1;
                break;
            }
            HuffmanTreeNode rem2 = queue.remove();
            HuffmanTreeNode tree = new HuffmanTreeNode(rem1.getWeight() + rem2.getWeight(), rem1, rem2);
            queue.add(tree);
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        // compressionRatio = ((float) getFileSize(textFile)) /
        // getSizeForCompressedFile(codeFile);
        // System.out.println("compression ratio for file: "+ textFile +" =
        // "+compressionRatio);
